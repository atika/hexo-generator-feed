...
env.addFilter('absoluteUrls', function(str, blog_url) {
  return str.replace(/(src|href)="(\S+)"/g, function(match, attr, link){
    // ça fonctionne mais voir pour les images commancant par /
    console.log(link+" >>> "+url.resolve(blog_url, link))
    return attr+'="'+url.resolve(blog_url, link)+'"';
  });
});
...
module.exports = function(locals) {
  ...
  // var url = config.url;
  var url = require('url').resolve(config.url, config.root);
